...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The address is guaranteed to be valid and contains a mutable reference to a `u32` value.
    // The caller has ensured that there are no other references to this memory location.
    unsafe {
